...
1.懒加载：
        基础懒加载：loading:'lazy'，好处是只用下载眼前所见的图片，并不是加载所有的图片。
        但网速慢时，会出现白屏图片加载卡顿

        高级懒加载：在img外层套一个div，div里放模糊的小尺寸图片，大概20寸，大小<1kb,当你打开界面时，
        率先加载这个1kb的模型图片
        编写一个JS脚本，如果img标签上放一个load的监听器，如果img加载完毕，就让img标签从透明度0到1，
        并编写相应的CSS
...
/* 
    HTML:
         input事件：用户在输入框打字时触发
         change事件：焦点离开输入框时触发。比如说你点击其他地方或按下Tab 键，使输入字段失去焦点
    React：
         onChange事件：是用于监听用户输入的变化，并且在每次输入时触发。它相当于 HTML 中的 input 事件
*/
